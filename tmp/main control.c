...
#define MOTOR_CHANNEL LEDC_CHANNEL_3
...
static const char *TAG = "Car_Camera";
...
{
    char buf[100];
    int ret, light_value, speed_value, motor_dir, switch_value;
    ...
    if (sscanf(buf, "Speed=%d", &speed_value) == 1)
    {
        printf("Speed value set to: %d", speed_value);
        ledc_set_duty(LEDC_MODE, MOTOR_CHANNEL, speed_value);
        ledc_update_duty(LEDC_MODE, MOTOR_CHANNEL);
    }
    if (sscanf(buf, "sw=%d", &switch_value) == 1)
    {
        if (switch_value)
        {
    ...
    if (sscanf(buf, "MoveCar=%d", &motor_dir) == 1)
    {
        switch (motor_dir)
        {
        case UP:
...
static void setupPins()
{
    ...
    ledc_channel.channel = MOTOR_CHANNEL;
    ledc_channel.gpio_num = SpeedPin;
    ledc_channel.duty = 150;
    ESP_ERROR_CHECK(ledc_channel_config(&ledc_channel));
